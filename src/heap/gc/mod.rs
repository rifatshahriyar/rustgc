...
//    USER_THREAD_PREPARE_FOR_GC.read().unwrap()();
	...
	while count < *N_MUTATORS.read().unwrap() - 1 {
        ...
        //println!("everyone stopped, gc will start");
...
fn block_current_thread(mutator: &mut ImmixMutatorLocal) {
    ...
    //println!("Mutator {} blocked", mutator.id());
    ...
    *count += 1;
    ...
    //println!("Mutator {} unblocked", mutator.id());
}
...
fn gc() {
    ...
    for elements in roots.iter() {
        ...
        if hash.write().unwrap().contains_key(&base) == true {
            let v = hash.write().unwrap()[&base];
            if v {
            }
            else {
                hash.write().unwrap().remove(&base);
